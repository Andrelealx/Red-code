...
const string connStr = @"Server=localhost,1433;Database=TicketPrime;User Id=sa;Password=TicketPrime@2024;TrustServerCertificate=True;";
...
app.MapPost("/api/reservas", async (ReservaReq req) => {
    ...
    // ID 11 / R1 — Integridade: validar existência do UsuarioCpf
    var usuarioExiste = await db.QuerySingleAsync<int>("SELECT COUNT(1) FROM Usuarios WHERE Cpf = @Cpf", new { Cpf = req.UsuarioCpf });
    if (usuarioExiste == 0) return Results.BadRequest("Erro: Usuário com este CPF não encontrado. Realize o cadastro antes de reservar.");

    // R1 — Integridade: validar existência do EventoId
    ...
    // ID 12 / R3 — Bloquear Overbooking (Capacidade Máxima)
    ...
    // ID 13 / R2 — Bloquear Cambistas: limite de 2 reservas por CPF no mesmo evento
    ...
    if (temReserva >= 2) return Results.BadRequest("Erro Crítico: Limite excedido. Permitido no máximo 2 ingressos por CPF por evento.");
    ...
    if (!string.IsNullOrEmpty(req.CupomUtilizado)) {
        ...
        // ID 08 / R4 — Motor de Cupons: aplica desconto somente se preço >= valor mínimo
    ...
    return Results.Created("/api/reservas", new { Mensagem = "Reserva concluída com sucesso!", ValorPago = valorFinal });
});

// HU-05: GET /api/reservas/{cpf} — Consultar reservas por CPF com JOIN
